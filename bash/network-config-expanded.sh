...
#  Creates the interfaces command. Uses the netstat -i (interface) command and the pipes
# it to an awk command, which specifices not to use select text with the '!/' command, and
# instructs bash to print only the first column
interfaces=$(netstat -i | awk '!/Kernel|Iface|lo/{print $1}')
...
# Utilizes the cat command, and prints all text and variables with the EOF commands
cat <<EOF
